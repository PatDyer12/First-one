...
def write(out, path=os.path.join(HERE, "card.html"), title="NFL Betting Model"):
    ...
    for sy in out.get("systems") or []:
        ...
        parts.append("<div class='grid'>" + "".join(f"<div class='stat'><b>{E(str(v))}</b><span>{E(k)}</span></div>" for k, v in (
            ("Record since 2011", sy["record"]), ("ROI since 2011", f"{sy['roi%']:+}%"),
            ("2025–26 record", sy["recent_record"]), ("2025–26 ROI", f"{sy['recent_roi%']:+}%"),
            ("2025–26 units", f"{sy['recent_units']:+}"))) + "</div>")
        seasons = " · ".join(f"{k}: {v:+}" for k, v in sy.get("by_season", {}).items())
        parts.append(f"<p class='note'>{E(sy['why'])}<br>Units by season: {E(seasons)}</p>")
        if sy["games"]:
